...
function twitterCallback2(twitters, mask) {
  ...
  for (var i=0; i<twitters.length; i++){
    ...
    //イメージ
    var twimage='<div id="img_container"><img src="'+twitters[i].avatar+'" width="48" height="48" onclick="selectID(\''+twitters[i].twname+'\')"><div class="small" onclick="alterFilter(\''+twitters[i].id_str+'\',\''+((visible)?"1":"0")+'\')">'+((visible)?"表示":"非表示")+'</div></div>';
    //名前・ID
    var nickname='<div id="twname"><a href='+twitters[i].link+' title="HPを開く">'+'<span  class="nickname">'+twitters[i].name+'</span></a> <a href="http://twitter.com/#!/'+username+'" title="ツイッターホームを開く">(@'+username+')</a></div>';
	...
	var tval=new Date();
    if(u_time(twitters[i].created_at)<tval.getTime()+tval.getTimezoneOffset()*60*1000-period*24*3600*1000) timestamp="<font color='crimzson'>"+timestamp+"</font>";

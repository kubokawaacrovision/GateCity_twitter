...
eObj.prototype.getTwTags=function(o){
	var n=o;
	while(n){
		if(n.id=="twavatar02") this.avatar=n;
		else if(n.id=="twname02") this.name=n;
		else if(n.id=="twtext02") this.text=n;
		else if(n.id=="twtime02") this.time=n;
		else if(n.id=="twlink02") this.link=n;
		else if(n.id=="twcategory02") this.category=n;
		if(n.hasChildNodes()){
...
function twitterCallback02(twitters, mask, redraw) {
  ...
  for (var i=0; i<twitters.length; i++){
	...
	var twuser=new eObj(document.getElementById("twuser02").cloneNode(true));
...
function filtering02(cat){
    ...
    for(i=0; i<statusHTML.length; i++){
        if(statusHTML[i][1]==cat || cat=='all' || i==0){
			...
			if(i==0){
				var twheader=document.getElementById("twheader").cloneNode(true);
				twheader.innerHTML='<h3 class="title_twitter">各店グループ公式アカウント</h3>\n<span class="date_txt_small">※掲載情報に関するお問い合わせは公式ホームページまでお願いします。</span>\n';
				twwrapper.appendChild(twheader);
			} else {

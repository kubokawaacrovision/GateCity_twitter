...
function filtering(cat){
    ...
    for(i=0; i<statusHTML.length; i++){
        if(statusHTML[i][1]==cat || cat=='all' || i==0){
			...
			if(i==0){
				var twheader=document.getElementById("twheader").cloneNode(true);
				twheader.innerHTML='<h3 class="title_twitter">大崎店　店長のつぶやき</h3>\n<span class="date_txt_small">※掲載情報に関するお問い合わせは各店舗までお願いします。</span>\n';
				twwrapper.appendChild(twheader);
			} else if(i==3){
				var twheader = document.getElementyBy('twheader').cloneNode(true);
				twheader.innerHTML='<h3 class="title_twitter" ></h3>\n<span class="date_txt_small">※掲載情報に関するお問い合わせはホームページまでお願いします。</span>\n';
				twwrapper.appendChild(twheader);
			} else {

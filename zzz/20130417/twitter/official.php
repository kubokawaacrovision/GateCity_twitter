...
function putExit2($tw, $cat, $fil, $option){
		echo 'var officialtw='.$tw.";\n";
		echo 'var filter2='.json_encode($fil).";\n";
	//	echo "var format2=".$option[0]['format'].";\n";
		echo "var period2='".$option[0]['period']."';\n";
		echo "var redraw2='".$option[0]['redraw']."';\n";
		echo 'twitterCallback3(officialtw, filter2, redraw2);';
		exit(1);
}

//main
$flock=new file_lock($cachepath2.'.lock');	//ロックオブジェト
if(!file_exists($cachepath2)){
	//キャッシュファイルが存在しなければ作成。初回のみの処理。
	$fh=fopen($cachepath2, "w");
...
//処理中キャッシュファイルをロックする
if($flock->lock($cachepath2)){
	if(filemtime($cachepath2)<time()-(3600/150)+1 || filesize($cachepath2)<1 || $_GET['refresh']=="true"){
		//最新つぶやきidの計算
		$json=readcache($cachepath2);
		...
		fputs($fh, $json_jsarr);
		fclose($fh);
		$flock->unlock($cachepath2);
		//NGリストの更新
		updateFilter($jsarr, $filter);
		//httpd出力・終了
		putExit2($json_jsarr, $category, $filter, $option);
	}
	//useCache:	//キャッシュ使用
	$json=readcache($cachepath2);
	$flock->unlock($cachepath2);
	...
	$json=json_encode($tw);
	//httpd出力
	putExit2($json, $category, $filter, $option);
} else {

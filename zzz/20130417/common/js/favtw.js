...
var statusHTML2=new Array();
var twwrapper=document.getElementById("left_contents");
var filtered="all";
...
loadingSpin.appendChild(spinner.el);
function filObj(fil){
	this.data=fil;
}
...
function twitterCallback2(twitters, mask, redraw) {
	if(loadingSpin){
		for(i=0, j=loadingSpin.children.length; i<j; i++){
			loadingSpin.removeChild(loadingSpin.lastChild);
		}
	}
  ...
  */
  for (var i=0; i<twitters.length; i++){
	...
	if(twitters[i].category.match(/^root$/i)) twuser.category.innerHTML="&nbsp";
		...
		while (twuser.category.hasChildNodes()) {
			twuser.category.removeChild(twuser.category.firstChild);
		}
	...
	atag.appendChild(img);
	while (twuser.link.hasChildNodes()) {
		twuser.link.removeChild(twuser.link.firstChild);
	}
  ...
  if(redraw){
	...
	setTimeout(function(){
	  	var officialphp=document.getElementById('official');
	  	var src=official.getAttribute('src');
	  	src.match(/^.*official\.php/);
	  	
		document.body.removeChild(official);
		var jsp=document.createElement('script');
		jsp.setAttribute('type', 'text/javascript');
		jsp.setAttribute('src', RegExp.lastMatch+'?'+(new Date()).getTime());
		jsp.setAttribute('charset', 'utf-8');
		jsp.setAttribute('id', 'official');
		document.body.appendChild(jsp);
	  }, redraw*1000);
  }
}
function twitterCallback3(twitters, mask, redraw) {
	  var filter=new filObj(mask);
	  statusHTML2 = new Array();
	  for(i=0; i<twitters.length-1; i++){
	  	for(j=i+1; j<twitters.length; j++){
	  		if(u_time(twitters[i].created_at)<u_time(twitters[j].created_at) || twitters[j].category=='root'){
	            if(twitters[i].category!='root'){
	...
	for (var i=0; i<twitters.length; i++){
		...
		twuser.link.appendChild(atag);
	    statusHTML2.push([twuser.elem, twitters[i].category]);
	  }
	filtering(filtered);
}
...
function filtering(cat){
    ...
    */
    ...
    for(i=0; i<statusHTML2.length; i++){
        if(statusHTML2[i][1]==cat || cat=='all' || i==0){
            twwrapper.appendChild(statusHTML2[i][0]);
			if(i==0){
				var twheader=document.getElementById("twheader").cloneNode(true);

...
function twitterCallback2(twitters, mask, redraw) {
  ...
  for (var i=0; i<twitters.length; i++){
	...
	if(twitters[i].category.match(/^root$/i)) twuser.category.innerHTML="&nbsp";
		...
		img.src="../../twitter/img/"+CATEGORY[twitters[i].category][1]+".gif";
	...
	var img=document.createElement('img');
	if(twitters[i].category.match(/^root$/i)) img.src="../../twitter/img/gco.gif";
	else img.src="../../twitter/img/date.gif";
	img.alt="店舗のページへ"; 
	img.className="ro";
	var atag=document.createElement('a');
    ...
    for(i=0; i<statusHTML.length; i++){
        if(statusHTML[i][1]==cat || cat=='all' || i==0){
			...
			if(i==0){
				...
				twlineimg.src="../../twitter/img/line.gif";

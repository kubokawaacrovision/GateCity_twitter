...
function twitterCallback2(twitters, mask, redraw) {
  ...
  for (var i=0; i<twitters.length; i++){
    ...
    var dval=new Date();
    if(u_time(twitters[i].created_at)<dval.getTime()+dval.getTimezoneOffset()*60*1000-period*24*3600*1000) continue;
...
function filtering(cat){
	...
	if(statusHTML[0][1]=='root') twheader.innerHTML='<h3 class="title_twitter">ゲートシティプラザのつぶやき</h3>';
	else twheader.innerHTML='<h3 class="title_twitter">ゲートシティプラザ　お店のつぶやき</h3>\n<span class="date_txt_small">※掲載情報に関するお問い合わせは、各店舗までお願いします。</span>\n';
  	twwrapper.appendChild(twheader);
	filtered=cat;
    for(i=0; i<statusHTML.length; i++){
        if(statusHTML[i][1]==cat || cat=='all' || statusHTML[i][1]=="root"){
            twwrapper.appendChild(statusHTML[i][0]);
			if(i==0 && statusHTML[i][1]=='root'){
				var twheader=document.getElementById("twheader").cloneNode(true);
				twheader.innerHTML='<h3 class="title_twitter">大崎店　店長のつぶやき</h3>\n<span class="date_txt_small">※掲載情報に関するお問い合わせは、各店舗までお願いします。</span>\n';
				twwrapper.appendChild(twheader);
			} else {
